...
int main( int argc, char **argv )
{
	...
	if(rank == 0)
	{
		...
		sprintf( graph_fname, "graphs/p%dd%dc%d_%s.png",
			   numtasks, num_dendrs, num_comps, time_str );
	...
	res[0] = y[0];
	double CurrentAccumulatedperTask;
	...
	for (t_ms = 1; t_ms < COMPTIME; t_ms++) 
	{
		...
		for (step = 0; step < STEPS; step++) 
		{
			soma_params[2] = 0.0;
			dendrite = rank;
			CurrentAccumulatedperTask = 0;
			//each process calculates current injected by more than 1 dendrite if number of dendrites is greater than number of processes
			while(dendrite<num_dendrs)
			{
				current = dendriteStep( dendr_volt[ dendrite ],
											...
											y[0] );
				dendrite+=numtasks;
				CurrentAccumulatedperTask+=current;
			}
			if(rank == 0)
			{
				soma_params[2] = CurrentAccumulatedperTask;
				double RecvBuf;
				int i;
				for(i=1;i<numtasks;i++)
				{
					//master process receives accumulated current from each process and calculates total current
					MPI_Recv(&RecvBuf, 1, MPI_DOUBLE, i, i, MPI_COMM_WORLD, &status);
					soma_params[2]+=RecvBuf;
				}
			}
			else
			{
				//slave processes send accumulated current to master
				MPI_Send( &CurrentAccumulatedperTask, 1, MPI_DOUBLE, 0, rank, MPI_COMM_WORLD );
			}
			if(rank == 0)
			{
			...
			if(rank == 0)
			{
				double sendbuffer = y[0];
				for(i = 1;i < numtasks;i++)
				{
					//master process sends updated y[0] value to all slave processes
					MPI_Send( &sendbuffer , 1, MPI_DOUBLE, i, i+55, MPI_COMM_WORLD );
				}
			}
			else
			{
				double RecvBuf;
				//slave process receives updated y[0] value from master process
				MPI_Recv(&RecvBuf, 1, MPI_DOUBLE, 0, 55+rank, MPI_COMM_WORLD, &status);
				y[0]=RecvBuf;
			}
